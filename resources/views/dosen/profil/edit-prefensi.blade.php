...
@section('content')
    ...
    <form method="POST" action="{{ route('dosen.profil.update_preferensi', $user->id) }}" class="bg-white border border-gray-200 rounded-lg p-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div class="col-span-2 sm:col-span-1">
                ...
                <input type="text" name="preferensi_lokasi" value="{{ old('preferensi_lokasi', $user->dosenPembimbing->preferensi_lokasi) }}"
            ...
            <div class="col-span-2 sm:col-span-1">
                ...
                <select name="jenis_magang_id" class="mt-2 w-full rounded-md border-gray-300 text-sm shadow-sm">
                    @foreach ($jenismagang as $jenis)
                        <option value="{{ $jenis->id }}" {{ $user->dosenPembimbing->jenis_magang_id == $jenis->id ? 'selected' : '' }}>
                            {{ $jenis->jenis_magang }}
                        </option>
                    @endforeach
                </select>
                ...
                @foreach ($skills as $skill)
                    <div class="flex items-center">
                        <input id="skill-{{ $skill->id }}" name="skills[]" value="{{ $skill->id }}"
                            ...
                            {{ in_array($skill->id, $dosen->skills->pluck('id')->toArray()) ? 'checked' : '' }}>
