        ...
        <div class="border-b border-gray-900/10 pb-12 p-6 bg-white border border-gray-200 rounded-lg">
            <div class="grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6 mb-4 ">
                ...
                <div class="sm:col-span-3">
                    ...
                    <div class="mt-2">
                        <input type="text" name="ipk" id="ipk" value="{{ old('ipk', $user->mahasiswa->ipk) }}"
                    ...
                    <select id="jenis_magang_id" name="jenis_magang_id"
                        ...
                        @foreach ($jenismagang as $jenis)
                            <option value="{{ $jenis->id }}"
                                {{ old('jenis_magang_id', $user->mahasiswa->jenis_magang_id) == $jenis->id ? 'selected' : '' }}>
                                {{ $jenis->jenis_magang }}
                            </option>
                        @endforeach
                    </select>
    ...
    <form method="POST" action="{{ route('mahasiswa.dokumen.update') }}" enctype="multipart/form-data">
        ...
        <div class="border-b border-gray-900/10 pb-12 p-6 bg-white border border-gray-200 rounded-lg">
            ...
            <div class="mt-6 flex items-center gap-x-6">
                <a href="#" class="text-sm font-semibold text-gray-900 hover:border border-gray-900 rounded-md px-3 py-2">Batal</a>
                <button type="submit" class="bg-indigo-600 hover:bg-indigo-500 rounded-md px-3 py-2 text-sm font-semibold text-white shadow-sm">Simpan</button>
            </div>
        </div>
    </form>
@endsection
