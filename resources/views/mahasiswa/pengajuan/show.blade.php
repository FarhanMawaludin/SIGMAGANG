...
<div class="overflow-x-auto relative rounded-lg border border-gray-200 bg-white">
    ...
    <div class="flex justify-center py-6">
        @if ($pengajuan->mahasiswa->user->photo ?? false)
            <img src="{{ asset('images/logo/' . $pengajuan->mahasiswa->user->photo) }}" alt="Foto {{ $pengajuan->mahasiswa->user->name }}"
                ...
                <td class="px-6 py-4">
                     <span class="inline-block px-2 py-1 text-xs font-semibold rounded 
                        {{ 
                            ...
                            ($pengajuan->status === 'accepted' ? 'bg-green-100 text-green-800' :
                            ($pengajuan->status === 'rejected' ? 'bg-red-100 text-red-800' :
                            ($pengajuan->status === 'completed' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800')))
                        }}">
